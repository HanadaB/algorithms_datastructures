...
private:
  ...
  // ....
  // vector representation of ACTrie
  
  
  struct NodeTrie { 
  int index; // Eindeutiger Index des Knotens im Trie.
  int suffix_link; // Zeiger auf den längsten passenden Suffix-Knoten.
  std::map<char, int> children; // Zuordnung von Zeichen zu Kindknoten.
  std::vector<int> needle_indices; // Needleindizes, die in diesem Knoten enden.
};
  std::vector<NodeTrie> ACVector; // Vektor-Darstellung des ACTrie
  std::string tree_recursion(int index) const; // Hilfsfunktion für die rekursive Baumdarstellung
  std::string query; // Aktuelle Query
  int current_search_index; // Aktueller Suchindex
  int query_pos; // Position in der Query
  std::vector<std::string> needleslist; // Liste der Muster
};
